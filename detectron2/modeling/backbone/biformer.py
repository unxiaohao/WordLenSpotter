...
class BiFormer(Backbone):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        for i in range(self.num_layers):
            ...
            stagen = f'stage{i+2}'
            if stagen in self.out_features:
                self._out_feature_channels[stagen] = embed_dim[i]
                self._out_feature_strides[stagen] = 4 * 2 ** i
            cur += depth[i]
...
def build_biformer_backbone(cfg, input_shape):
    """
...
@BACKBONE_REGISTRY.register()
def build_biformer_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    """
    bottom_up = build_biformer_backbone(cfg, input_shape)
...
@BACKBONE_REGISTRY.register()
def build_biformer_bifpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
