...
class TextEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if "totaltext" in dataset_name:
            ...
            self.lexicon_type = 3  # none lexicon
            self._text_eval_gt_path = "datasets/evaluation/gt_icdar2015.zip"
            self._word_spotting = False
            self.dataset_name = "icdar2015"
        elif "dstd1500" in dataset_name:
            # self.lexicon_type = None   # none lexicon
            self._text_eval_gt_path = "datasets/evaluation/test_gt.zip"
            self._word_spotting = False
            self.dataset_name = "dstd1500"
    ...
    def sort_detection(self, temp_dir):
        ...
        if "totaltext" in self.dataset_name:
            if not self.lexicon_type == None:
                ...
                for line in lexicon_fid.readlines():
                    line=line.strip()
                    lexicon.append(line)
        elif "dstd1500" in self.dataset_name:
            if not self.lexicon_type == None:
                lexicon_path = 'datasets/dstd1500/weak_voc_new.txt'
                lexicon_fid=open(lexicon_path, 'r')
                pair_list = open('datasets/dstd1500/weak_voc_pair_list.txt', 'r')
                pairs = dict()
                for line in pair_list.readlines():
                    line=line.strip()
                    word = line.split(' ')[0].upper()
        ...
        for i in files:
            ...
            for iline, line in enumerate(fin):
                ...
                pts = outstr
                if "icdar2015" in self.dataset_name or "dstd1500" in self.dataset_name:
                    outstr = outstr + ',' + rec
                else:
                    outstr = outstr + ',####' + rec
