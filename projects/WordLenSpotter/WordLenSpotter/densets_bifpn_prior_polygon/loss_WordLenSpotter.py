...
class SetCriterion(nn.Module):
    ...
    def loss_prior(self, outputs, targets, indices, num_boxes, mask_encoding):
        assert 'pred_prior' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_prior = outputs['pred_prior'][idx]
        target_prior = torch.cat([t['gt_prior'][i] for t, (_, i) in zip(targets, indices)], dim=0)

        losses = {}
        loss_prior = F.smooth_l1_loss(src_prior, target_prior, reduction='none')
        losses['loss_prior'] = loss_prior.sum() / num_boxes
        return losses
    
    def loss_polygon(self, outputs, targets, indices, num_boxes, mask_encoding):
        assert 'pred_polygons' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_polygon = outputs['pred_polygons']
        target_polygon = torch.cat([t['gt_polygons_feature'] for t in targets], dim=0)
        
        losses = {}
        eps = 1e-5
        src_polygon_ = src_polygon.flatten(1)
        target_polygon_ = target_polygon.flatten(1)
        intersection = (src_polygon_ * target_polygon_).sum(dim=1)
        union = (src_polygon_ ** 2.0).sum(dim=1) + (target_polygon_ ** 2.0).sum(dim=1) + eps
        loss_polygon_dice = 1. - (2 * intersection / union)
        losses['loss_polygon_dice'] = loss_polygon_dice.sum() / num_boxes
...
class HungarianMatcher(nn.Module):
    def __init__(self, cfg, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, cost_mask: float = 1, cost_prior:float = 1):
        super().__init__()
    ...
    def forward(self, outputs, targets, mask_encoding):
        ...
        # cost_prior
        tgt_prior = torch.cat([v["gt_prior"] for v in targets])
        out_prior = outputs["pred_prior"].flatten(0, 1)
        cost_prior = torch.cdist(out_prior, tgt_prior, p=1)
        
        # Final cost matrix
        C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou \
            + self.cost_mask * cost_mask + self.cost_prior * cost_prior
